...
try {
  ...
  try {
    ...
    console.error(
      '❌ Failed to install Linux dependency even with force flag:',
      forceError.message,
    );
    ...
    try {
      ...
      execSync(
        'npm install https://registry.npmjs.org/@rollup/rollup-linux-x64-gnu/-/rollup-linux-x64-gnu-4.9.5.tgz --no-save',
        { stdio: 'inherit', cwd: rootDir },
      );
      console.log('✅ Successfully installed Linux Rollup dependency with direct approach');
    } catch (directError) {
      console.error(
        '❌ All approaches failed. Please check your network connection and npm configuration.',
      );
      process.exit(1);
    }
  }
