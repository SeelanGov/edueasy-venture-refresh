...
const requiredFiles = [
  ...
  { path: 'vite.config.ci.ts', description: 'Vite CI configuration' }
];
...
try {
  ...
  const requiredScripts = [
    'build', 'build:ci', 'build:safe', 'test', 'check-env', 'type-check', 'lint',
    'fix-rollup', 'deploy:staging', 'deploy:production', 'ci:status'
  ];
...
try {
  ...
  if (fs.existsSync(workflowPath)) {
    ...
    const hasStagingJob = workflowContent.includes('deploy-staging:');
    const hasProductionJob = workflowContent.includes('deploy-production:');
    
    console.log('\n🔍 GitHub workflow checks:');
    console.log(`  - Build job: ${hasBuildJob ? '✅' : '❌'}`);
    console.log(`  - Staging deployment job: ${hasStagingJob ? '✅' : '❌'}`);
    console.log(`  - Production deployment job: ${hasProductionJob ? '✅' : '❌'}`);
    
    if (!hasBuildJob || !hasStagingJob || !hasProductionJob) {
