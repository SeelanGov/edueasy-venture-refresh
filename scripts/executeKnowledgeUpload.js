...
const SUPABASE_SERVICE_KEY =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBlbnN2YW10Zmp0cHNhb2VmbGJ4Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc0MzgzNzI5NywiZXhwIjoyMDU5NDEzMjk3fQ.5C2ffCUxF_85oNJRWXJr9_xmfnvFLl5nh6mD5qUTNQY';
...
const knowledgeModules = [
  {
    ...
    htmlContent:
      '<h1>Bursaries and Scholarships</h1><p>Comprehensive guide to funding opportunities in South Africa.</p>',
  },
  {
    ...
    htmlContent:
      '<h1>Critical and Scarce Skills</h1><p>Explore high-demand career opportunities in South Africa.</p>',
  },
  {
    ...
    htmlContent:
      '<h1>Future Work Trends</h1><p>Prepare for emerging career opportunities and industry changes.</p>',
  },
  {
    ...
    htmlContent:
      '<h1>NSC Curriculum</h1><p>Complete guide to Matric requirements and subjects.</p>',
  },
  {
    ...
    jsonContent: {
      ...
      language: 'en',
      summary:
        'The National Student Financial Aid Scheme (NSFAS) provides funding for South African students.',
    ...
    htmlContent:
      '<h1>NSFAS Application</h1><p>Step-by-step guide to applying for NSFAS funding.</p>',
  },
  {
    ...
    htmlContent:
      '<h1>Recognition of Prior Learning</h1><p>Turn your experience into formal qualifications.</p>',
  },
  {
    ...
    jsonContent: {
      ...
      language: 'en',
      summary:
        'The National Qualifications Framework (NQF) organizes South African qualifications.',
    ...
    htmlContent:
      '<h1>SAQA Qualifications Framework</h1><p>Understanding the NQF levels and qualification pathways.</p>',
  },
  {
    ...
    htmlContent:
      '<h1>SETA Learnerships</h1><p>Practical training programs for career development.</p>',
  },
  {
    ...
    jsonContent: {
      ...
      language: 'en',
      summary:
        'Explore Technical and Vocational Education and Training (TVET) colleges with EduEasy.',
  ...
  },
  {
    ...
    htmlContent:
      '<h1>University Applications</h1><p>Complete guide to university applications in South Africa.</p>',
  },
];

async function uploadKnowledgeBase() {
  console.log(
    '🚀 Starting Knowledge Base Upload with IMMEDIATE VERIFICATION (Anti-Waste Protocol)...\n',
  );
  ...
  for (const moduleData of knowledgeModules) {
    ...
    try {
      ...
      const jsonUpload = await supabase.storage.from(STORAGE_BUCKET).upload(jsonPath, jsonBuffer, {
      ...
      if (jsonUpload.error) {
        ...
        results.push({
          ...
          success: false,
          error: jsonUpload.error.message,
        });
        continue;
      }
      ...
      if (jsonListError || !jsonVerifyList || jsonVerifyList.length === 0) {
        console.log(
          `    ❌ VERIFICATION FAILED - No files found in ${moduleData.module}:`,
          jsonListError?.message || 'Empty directory',
        );
        results.push({
          module: moduleData.module,
          success: false,
          error: 'JSON verification failed - directory empty',
        });
        continue;
      }

      const jsonFile = jsonVerifyList.find(
        (file) => file.name === `${moduleData.module.replace('_framework', '')}.json`,
      );
      if (!jsonFile) {
        console.log(`    ❌ VERIFICATION FAILED - JSON file not found in ${moduleData.module}`);
        console.log(
          `    📋 Found files:`,
          jsonVerifyList.map((f) => f.name),
        );
        results.push({
          module: moduleData.module,
          success: false,
          error: 'JSON file not found in verification',
        });
        continue;
      }

      console.log(
        `    ✅ JSON VERIFIED - Found in storage: ${jsonFile.name} (${jsonFile.metadata?.size || 'unknown'} bytes)`,
      );
      totalUploaded++;
      ...
      const htmlUpload = await supabase.storage.from(STORAGE_BUCKET).upload(htmlPath, htmlBuffer, {
      ...
      if (htmlListError || !htmlVerifyList || htmlVerifyList.length === 0) {
        console.log(
          `    ❌ VERIFICATION FAILED - No files found in ${moduleData.module}:`,
          htmlListError?.message || 'Empty directory',
        );
        results.push({
          module: moduleData.module,
          success: false,
          error: 'HTML verification failed - directory empty',
        });
        continue;
      }

      const htmlFile = htmlVerifyList.find(
        (file) => file.name === `${moduleData.module.replace('_framework', '')}.html`,
      );
      if (!htmlFile) {
        console.log(`    ❌ VERIFICATION FAILED - HTML file not found in ${moduleData.module}`);
        console.log(
          `    📋 Found files:`,
          htmlVerifyList.map((f) => f.name),
        );
        results.push({
          module: moduleData.module,
          success: false,
          error: 'HTML file not found in verification',
        });
        continue;
      }

      console.log(
        `    ✅ HTML VERIFIED - Found in storage: ${htmlFile.name} (${htmlFile.metadata?.size || 'unknown'} bytes)`,
      );
      console.log(
        `    📊 Module complete: ${htmlVerifyList.length} files verified in ${moduleData.module}`,
      );
      totalUploaded++;

      // Insert/update database record
      console.log(`  📝 Updating database index for ${moduleData.module}...`);
      const { data: dbData, error: dbError } = await supabase.from('thandi_knowledge_index').upsert(
        {
          module: moduleData.module,
      ...
      if (dbSelectError || !dbVerify) {
        console.log(
          `    ❌ DATABASE VERIFICATION FAILED for ${moduleData.module}:`,
          dbSelectError?.message || 'Record not found',
        );
        results.push({
          module: moduleData.module,
          success: false,
  ...
  if (indexError) {
    ...
    indexData.forEach((entry) => {
      ...
      console.log(
        `  ${status} ${entry.module} (JSON: ${jsonExists ? '✅' : '❌'}, HTML: ${htmlExists ? '✅' : '❌'})`,
      );
    });
  }
}
