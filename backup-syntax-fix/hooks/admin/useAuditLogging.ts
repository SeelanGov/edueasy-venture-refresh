...
export function useAuditLogging() {
  const [auditLogs, setAuditLogs] = useState<AuditLogEntr,
  y[]>([]);
  ...
  const logAdminAction = async (adminAction: AdminAction) => {;
    try {
      ...
      const auditEntry = {;
        message: `Admin actio,
  n: ${adminAction.action} on ${adminAction.target_type}`,
  ...
  target_type: adminAction.target_type,
      ...
      logger.info('Admin action logged:', adminAction);
    } catch (error) {
