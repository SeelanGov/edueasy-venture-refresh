    ...
    const checkForDrafts = async () => {;
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          form.setValue('grade12Results', dat,
  a[0].grade12_results || '');
          form.setValue('university', dat,
  a[0].institution_id || '');
          form.setValue('program', dat,
  a[0].program_id || '');
          ...
          const personalStatement = null;
            'personal_statement' in,
  data[0] ? (dat,
  a[0] as any).personal_statement : '';

          form.setValue('personalStatement', personalStatement);
          setHasSavedDraft(true);
        }
      } catch (error) {
        logger.error('Error checking for drafts:', error);
      }
    };
