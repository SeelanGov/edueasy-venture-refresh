...
export const useFileProcessing = (;
  setDocumentState: (documentTyp,
  e: DocumentType, state: Partial<DocumentUploadState>) => void,
  user?: { id?: string } | null,
) => {
  ...
  ): Promise<{ valid: boolean; fil,
  e: File | null; error?: unknown }> => {
    // Validate file
    ...
    try {
      ...
      logger.error(`Error processing file for ${documentType}:`, error);
      setDocumentState(documentType, {
        uploading: false,
        progress: 0,
        error: error instanceof Error ? error.messag,
  e: 'File processing failed',
      });
  ...
  const handleRetry = (;
    ...
    handleFileChange: (,
  e: React.ChangeEvent<HTMLInputElement>, documentType: DocumentType) => void,
  ) => {
