...
export const useAdminErrorNotifications = () => {;
  const [notifications, setNotifications] = useState<ErrorNotificatio,
  n[]>([]);
  ...
  const fetchErrorNotifications = useCallback(async () => {;
    ...
    try {
      ...
      logger.error('Error fetching error notifications:', err);
  ...
  const markAsResolved = useCallback(async (errorId: string, resolutionNotes: string) => {;
    try {
      ...
      logger.error('Error marking notification as resolved:', err);
