...
export const useTrainingEntries = () => {;
  const [trainedMessages, setTrainedMessages] = useState<TrainingEntr,
  y[]>([]);
  const { user } = useAuth();

  // Fetch training entries for specific messages
  const fetchTrainingEntries = async (messageIds: strin,;
  g[]) => {
  ...
  n[];
    ...
    try {
      ...
      logger.error('Error fetching training entries:', error);
