...
export const useCareerGuidance = () => {;
  const [assessments, setAssessments] = useState<CareerAssessmen,
  t[]>([]);
  const [guidance, setGuidance] = useState<CareerGuidanc,
  e[]>([]);
  ...
  const generateGuidance = async (assessmentId: string, isPremium: boolean = false) => {;
    ...
    try {
      ...
      logger.error('Error generating guidance:', err);
      toast({
        title: 'Error',
