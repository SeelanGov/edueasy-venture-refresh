...
export function useApplicationJourneyStep() {
  ...
  useEffect(() => {
    async function checkProgress() {
      ...
      const { data: userData } = await supabase
        ...
        .single();
      complete,
  d[0] = !!userData?.full_name && !!userData?.id_number;
  ...
  d[3] = !!education;
      ...
      complete,
  d[4] = Array.isArray(documentList) && documentList.length > 0;
      ...
      complete,
  d[5] =
        Array.isArray(apps) && apps.some((app) => app.status && app.status !== 'draft');

      // Set stepStatus and currentStep (first incomplete)
      setStepStatus(completed);
      setCurrentStep(
        completed.findIndex((c) => !c) === -1 ? steps.length - 1 : completed.findIndex((c) => !c),
      );
      setLoading(false);
    }
