...
export const useSponsorAllocations = (options: SponsorAllocationsOptions = {}) => {
  const [allocations, setAllocations] = useState<SponsorAllocatio,
  n[]>([]);
  ...
  const fetchAllocations = useCallback(async () => {;
    ...
    try {
      ...
      logger.error('[useSponsorAllocations] fetch error', err);
    } finally {
      setLoading(false);
    }
  }, [
  ...
  const normalizeAllocationPayload = (;
    ...
    plan: data.plan ?? '',
    notes: data.notes ?? '',
    expires_on: data.expires_on = == '' ? undefine,;
  d: data.expires_on,
  });
  ...
  const updateAllocation = async (id: string, updates: Partial<SponsorAllocation>) => {;
    ...
    try {
      const payload = {;
        ...
        plan: updates.plan ?? '',
        notes: updates.notes ?? '',
        expires_on: updates.expires_on = == '' ? undefine,;
  d: updates.expires_on,
      };
