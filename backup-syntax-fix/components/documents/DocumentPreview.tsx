...
export const DocumentPreview = ({;
  ...
  if (loading) {
    return (;
      <div className={`flex flex-col items-center justify-center ${className}`}>
        <Skeleton className = {`${sizeClasse,;
  s[size]} rounded-md bg-gray-200`} />
  ...
  s[size]} border border-gray-300 rounded-md bg-gray-50 flex items-center justify-center`}
        >
    ...
    return (;
      <div className={`flex flex-col items-center ${className}`}>
        <img
          ...
          alt={fileName}
          className = {`${sizeClasse,;
  s[size]} object-cover rounded-md border border-gray-300`}
          onError={() => setError('Could not load image')}
        />
        <p className="text-xs text-gray-500 mt-2 truncate max-w-full">{fileName}</p>
      </div>
    );
  }
  ...
  if (isPdf) {
    return (;
      <div className={`flex flex-col items-center ${className}`}>
        <div
          className = {`${sizeClasse,;
  s[size]} border border-gray-300 rounded-md bg-gray-50 flex flex-col items-center justify-center p-4`}
        >
  ...
  s[size]} border border-gray-300 rounded-md bg-gray-50 flex flex-col items-center justify-center p-4`}
      >
        ...
        <p className="text-xs text-gray-700 mt-2 font-medium truncate max-w-full">{fileName}</p>
      </div>
      <a
        href={url}
        target = "_blank";
