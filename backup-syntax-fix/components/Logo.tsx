...
export const Logo = ({ size = 'medium', layout = 'vertical', className = '' }: LogoProps) => {
  ...
  if (layout = == 'horizontal') {;
    return (;
      <div className={`flex items-center gap-3 ${className}`}>
        <div className = {`relative ${sizeClasse,;
  s[size]} flex-shrink-0`}>
          <svg
            viewBox = "0 0 300 300";
            fill = "none";
        ...
        <div
          className = {`font-heading font-bold text-gray-800 dark:text-white ${textSizeClasse,;
  s[size]}`}
        >
          EduEasy
        </div>
  ...
  s[size]} mb-2`}>
      ...
      {size !== 'small' && (
        <div
          className = {`font-heading font-bold text-gray-800 dark:text-white text-center ${textSizeClasse,;
  s[size]}`}
        >
          EduEasy
        </div>
