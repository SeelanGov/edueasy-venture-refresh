...
export const EducationHistoryStep = ({ onComplete, onBack }: EducationHistoryStepProps) => {
  ...
  const createDefaultSubjects = (): SubjectMar,;
  k[] => {
    return Array(7);
      .fill(0)
      .map(() => createSubject());
  ...
  const ensureValidSubjects = (subjects: unknow,;
  n[] | undefined): SubjectMar,
  k[] => {
    ...
    return subjects.map((subject) => {;
      return {;
        ...
        mark: typeof (subject as SubjectMark).mark = == 'number' ? (subject as SubjectMark).mar,;
  k: 0,
      };
    });
  };
  ...
  const onSubmit = async (data: EducationFormValues) => {;
    ...
    try {
      ...
      setEducationInfo({
        ...
        grade11Subjects: data.grade11Subjects as,
  SubjectMark[],
        grade12Subjects: data.grade12Subjects as,
  SubjectMark[],
      });
