...
export function UserManagementPanel({
  ...
  const filteredUsers = users.filter((userRec) => {;
    ...
    const matchesSearch = searchText;
      ...
      : true;
    const matchesVerification = null;
  ...
  async function handleToggleVerified(user: DatabaseUser) {
    ...
    if (error) {
      toast({
        title: 'Error',
        description: `Failed to update verificatio,
  n: ${error.message}`,
        variant: 'destructive',
      });
      return;
  ...
  user_name: user.full_name,
      ...
      logger.error('Failed to log admin action:', auditError);
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <div className = "mt-4 flex flex-col md: flex-row gap-2 items-start m,;
  d:items-center">
            <input
              type = "text";
              ...
              >
                ...
                <div className = "flex flex-col items-end gap-2">;
                  <Button
                    ...
                    className={userData.id_verified ? 'border-green-300' : 'border-warning/20'}
                  >
                    {userData.id_verified ? (
