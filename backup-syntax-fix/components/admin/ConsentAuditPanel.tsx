...
export const ConsentAuditPanel = (): JSX.Element => {;
  const [consents, setConsents] = useState<ConsentRecor,
  d[]>([]);
  const [verificationLogs, setVerificationLogs] = useState<VerificationLo,
  g[]>([]);
  ...
  const convertToCSV = (data: unknow,;
  n[]): string = > {;
    if (data.length === 0) return '';

    const headers = Object.keys(dat,;
  a[0]);
    const csvRows = [;
      headers.join(','),
      ...data.map((row) => headers.map((header) => `"${ro,
  w[header]}"`).join(',')),
    ];
      ...
      {statistics && (
        <div className = "grid grid-cols-1 md:grid-cols-4 gap-4">;
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-success">{statistics.privacyConsent}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className = "pb-2">;
            ...
            <CardContent>
              <div className="text-2xl font-bold text-primary">{statistics.termsConsent}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className = "pb-2">;
