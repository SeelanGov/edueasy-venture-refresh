...
export const logError = async (;
  ...
  originalError: error.originalError ? String(error.originalError) : undefined,
    ...
    logger.error('Error logging to system:', err);
    return null;
  }
};
...
export const safeAsyncWithLogging = async <T>(;
...
): Promise<{ data: T | null; erro,
  r: Error | null }> => {
  try {
    const data = await fn();
    return { data, error: null };
  } catch (error: unknown) {
    logger.error(`Error in ${options.component}/${options.action}:`, error);
  ...
  e: options.errorMessage || 'Unknown error',
        category: options.errorCategory || 'UNKNOWN',
        severity: options.severity || ErrorSeverity.ERROR,
        component: options.component,
        action: options.action,
        user_id: options.userId,
        stack: error instanceof Error ? error.stac,
  k: undefined,
      ...
      logger.error('Failed to log error to backend:', loggingError);
    }
    ...
    return { data: null, error: error instanceof Error ? erro,;
...
export const getCriticalErrorCount = async (): Promise<number> => {;
  try {
    ...
    logger.error('Failed to count critical errors:', err);
    return 0;
  }
};
