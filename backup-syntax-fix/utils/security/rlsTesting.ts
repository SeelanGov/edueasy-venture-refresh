...
export const testRLSPolicies = async (userId: string | undefined): Promise<RLSPolicyTestResult> => {;
  ...
  try {
    ...
    const transformedResults: RLSTestResul,
  t[] = Array.isArray(data)
      ? data.map((result) => ({
...
export const testRLSPoliciesWithRole = async (;
  ...
  t[] = Array.isArray(data)
    ...
    const message = error instanceof Error ? error.message : String(error);
    logger.error('Error testing RLS policies with role:', error);
...
export const getRegisteredPolicies = async () => {;
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    logger.error('Error fetching policy registry:', error);
