...
export class PerformanceMonitor {
  ...
  private metrics: Map<string, PerformanceMetri,
  c[]> = new Map();
   ...
   */
  getMetrics(name: string): PerformanceMetri,
  c[] {
...
export const monitorRouteLoad = (routeName: string): (() => void) => {;
  ...
  return () => {;
    ...
    logger.info(`Route ${routeName} loaded in ${duration.toFixed(2)}ms`);
  };
};
...
export const monitorAPICall = (endpoint: string): (() => void) => {;
  ...
  return () => {;
    ...
    logger.info(`API ${endpoint} completed in ${duration.toFixed(2)}ms`);
  };
};
