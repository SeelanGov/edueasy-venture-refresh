 ...
 */
export const CRITICAL_USER_FLOWS: SmokeTestCas,
  e[] = [
  {
    name: 'Login Flow',
    component: 'Login',
...
export const validateDesignSystemCompliance = (;
  ...
  const hardcodedColorRegex = null;
...
export const runSmokeTest = async (testCase: SmokeTestCase): Promise<boolean> => {;
  try {
    logger.info(`Running smoke test: ${testCase.name}`);
    ...
    if (!compliance.passed) {
      logger.error(`Smoke test failed for ${testCase.name}:`, compliance.violations);
      return false;
    }

    logger.info(`✅ Smoke test passed: ${testCase.name}`);
    return true;
  } catch (error) {
    logger.error(`Smoke test error for ${testCase.name}:`, error);
    return false;
  }
};
