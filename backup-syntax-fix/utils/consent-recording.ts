...
export async function recordUserConsents(userId: string, consents: ConsentData): Promise<void> {
  const consentRecords: ConsentRecor,
  d[] = [
    {
      user_id: userId,
 ...
 */
export async function getUserConsentHistory(userId: string): Promise<ConsentRecor,
  d[]> {
  const { data, error } = await supabase
    .from('user_consents')
    .select('*')
...
function getDefaultConsentText(consentType: string): string {
  ...
  return (;
    consentText,
  s[consentType] || 'I consent to the processing of my data for the specified purpose.'
  );
}
...
async function getClientIP(): Promise<string | undefined> {
  try {
    ...
    logger.warn('Could not determine client IP:', error);
    return undefined;
  }
}
...
export async function getConsentStatistics(): Promise<{
  ...
  if (error) {
    logger.error('Failed to fetch consent statistics:', error);
