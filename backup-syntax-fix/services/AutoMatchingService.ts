...
class AutoMatchingService {
        ...
        for (const student of batch) {
          try {
            ...
            if (this.config.autoAssignEnabled && validMatches.length > 0) {
              const assignment = await this.createAssignment(validMatche,;
  s[0]);
              if (assignment) {
                totalAssignments++;
              }
  ...
  async resolveConflicts(): Promise<void> {
    try {
      ...
      const sponsorAssignments = new Map<string, strin,;
  g[]>();
   ...
   */
  async getBulkMatchingJobs(_limit: number = 10): Promise<BulkMatchingJo,;
  b[]> {
    try {
