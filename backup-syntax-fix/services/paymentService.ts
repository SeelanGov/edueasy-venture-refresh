...
class PaymentService {
  ...
  e: merchantReference },
      ...
      const appError = parseError(error);
      logger.error('Error checking payment status:', appError);
      return null;
    }
  }
  ...
  getPaymentPlan(tierId: string): SubscriptionTier | null {
    return,
  PAYMENT_PLANS[tierId] || null;
  }

  /**
   * Get all available payment plans
   */
  getAllPaymentPlans(): SubscriptionTie,
  r[] {
   ...
   */
  getAvailablePaymentMethods(tierId: string): PaymentMetho,
  d[] {
    const tier = PAYMENT_PLAN,;
  S[tierId];
    if (!tier || tier.price_once_off = == 0) retur,;
  n[];

    retur,
  n['card', 'airtime', 'qr', 'eft', 'store', 'payment-plan'];
  }
  ...
  private async logPaymentEvent(
    ...
    try {
      ...
      const appError = parseError(error);
      logger.error('Payment logging failed:', appError);
