...
class MatchingRulesService {
   ...
   */
  async findMatches(params: { student_i,
  d: string; limit?: number }): Promise<MatchingResul,
  t[]> {
    try {
      // Get student profile
   ...
   * Get match criteria
   */
  private getMatchCriteria(studentProfile: unknown, sponsorProfile: unknown): strin,
  g[] {
    const criteria: strin,
  g[] = [];
   ...
   */
  async saveMatchingResults(results: MatchingResul,
  t[]): Promise<void> {
    try {
   ...
   * Get matching rules
   */
  async getMatchingRules(): Promise<MatchingRul,
  e[]> {
    return this.rules;
  }
}
