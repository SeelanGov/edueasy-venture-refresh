...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Calling VerifyID API for user ${user_id} with ID ending in ${national_id.slice(-4)}`,
    );
...
async function checkRateLimit(
  ...
  if (existing) {
    ...
    const firstAttempt =
      existing.first_attempt_at > oneHourAgo ? existing.first_attempt_at : new Date().toISOString();
    ...
    await supabase.from('verification_rate_limits').insert({
