  ...
  try {
    ...
    if (!ocrResult.ParsedResults || ocrResult.ParsedResults.length === 0) {
      return {
        ...
        failureReason:
          'Could not extract any text from the document. Please upload a clearer image.',
        extractedText: null,
    ...
    const confidenceFromAPI = parseFloat(
      ocrResult.ParsedResults[0].TextOverlay?.Lines?.[0]?.Words?.[0]?.WordText || '0',
    );
    const confidence =
      confidenceFromAPI > 0 ? confidenceFromAPI : estimateConfidence(extractedText);
...
function validateIdDocument(
  ...
  const nameMatch =
    text.match(/(?:surname|last\s*name|family\s*name)[\s:]+([A-Za-z\s]+)/i) ||
    text.match(/(?:name)[\s:]+([A-Za-z\s]+)/i);
  const dobMatch =
    text.match(/(?:birth|born|dob)[\s:]+(\d{2}[-./]\d{2}[-./]\d{2,4})/i) ||
  ...
  if (!idNumberMatch) {
    return {
      ...
      validationResults,
      failureReason:
        'Could not detect a valid ID number on the document. Please upload a clearer image.',
      extractedText: text,
 ...
 */
function validateGrade12Results(
  text: string,
  confidence: number,
): {
  status: 'approved' | 'rejected' | 'request_resubmission';
  ...
  const studentIdMatch = text.match(
    /(?:student\s*id|learner\s*id|candidate\s*number)[\s:]+([A-Z0-9\s]+)/i,
  );
  ...
  if (!subjectMatches || subjectMatches.length < 2) {
    return {
      ...
      validationResults,
      failureReason:
        'Could not detect subject results on the document. Please ensure this is a grade results document.',
      extractedText: text,
      extractedFields: Object.keys(extractedFields).length > 0 ? extractedFields : null,
    };
  }
 ...
 */
function validateGrade11Results(
  text: string,
  confidence: number,
): {
  status: 'approved' | 'rejected' | 'request_resubmission';
 ...
 */
function validateProofOfResidence(
  text: string,
  confidence: number,
): {
  status: 'approved' | 'rejected' | 'request_resubmission';
  ...
  const hasAddress = text.match(
    /(?:\b(?:address|street|avenue|road|lane|drive|boulevard|crescent)\b)/i,
  );
  const hasPostalCode = text.match(/\b\d{4,5}\b/); // Standard postal code format
  const hasName = text.match(
    /(?:\b(?:name|resident|customer|client|account holder)\b[\s:]+)([A-Za-z\s]+)/i,
  );
  const hasDate = text.match(
    /(?:\b(?:date|issued|statement date)\b[\s:]+)(\d{1,2}[-./]\d{1,2}[-./]\d{2,4})/i,
  );
  const hasUtilityProvider = text.match(
    /(?:\b(?:municipality|utility|provider|council|company)\b[\s:]+)([A-Za-z\s]+)/i,
  );
  ...
  if (hasAddress) {
    ...
    const addressMatch = text.match(
      /(?:\b(?:address|residential|physical|home)[\s:]+)([A-Za-z0-9\s,./-]+)/i,
    );
    if (addressMatch) {
  ...
  if (!hasAddress) {
    return {
      ...
      validationResults,
      failureReason:
        'Could not detect address information on the document. Please ensure this is a proof of residence document.',
      extractedText: text,
      extractedFields: Object.keys(extractedFields).length > 0 ? extractedFields : null,
    };
  }
