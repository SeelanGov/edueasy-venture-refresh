...
async function handleListOrphaned(supabase: any, adminContext: AdminContext): Promise<Response> {
  ...
  const orphanedThreshold = new Date(
    Date.now() - ORPHANED_PAYMENT_THRESHOLD_HOURS * 60 * 60 * 1000,
  );
...
async function handleUserRecoveryCheck(supabase: any, userEmail: string): Promise<Response> {
...
serve(async (req) => {
  ...
  try {
    const { action, payment_id, user_id, user_email, resolution_notes }: PaymentRecoveryRequest =
      await req.json();
    ...
    switch (action) {
      ...
      case 'link_payment':
        ...
        return await handleLinkPayment(
          supabase,
          adminContext,
          payment_id,
          user_id,
