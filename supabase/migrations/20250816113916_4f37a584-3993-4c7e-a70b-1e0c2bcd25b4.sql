-- Create admin-only functions for payment analytics access
-- This replaces direct view access with controlled function access

-- Function to get payment monitoring data (admin only)
...
AS $$
BEGIN
  -- Only admins can access payment monitoring data
...
-- Function to get payment method analytics (admin only)
...
AS $$
BEGIN
  -- Only admins can access payment analytics
  IF NOT public.is_admin(auth.uid()) THEN
    RAISE EXCEPTION 'Access denied. Admin privileges required.';
  END IF;
