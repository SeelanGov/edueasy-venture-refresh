...
export const usePaymentRecovery = () => {
  ...
  const executeRecoveryAction = async (options: PaymentRecoveryOptions): Promise<PaymentRecoveryResponse> => {
    setLoading(true)
    setError(null)
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/payment-recovery`, {
        method: 'POST',
        headers: {
