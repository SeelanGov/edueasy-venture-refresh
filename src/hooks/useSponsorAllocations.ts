...
export const useSponsorAllocations = (
  options: SponsorAllocationsOptions = {}
) => {
  ...
  const createAllocation = async (
    ...
    try {
      ...
      setError(
        (err as Error).message ? `Failed to create allocation: ${(err as Error).message}` : "Failed to create allocation"
      );
      return null;
    } finally {
      setLoading(false);
  ...
  const updateAllocation = async (
    id: string,
    updates: Partial<SponsorAllocation>
  ) => {
    setLoading(true);
    setError(null);
    try {
      ...
      setError(
        (err as Error).message ? `Failed to update allocation: ${(err as Error).message}` : "Failed to update allocation"
      );
      return null;
    } finally {
      setLoading(false);
  ...
  const deleteAllocation = async (id: string) => {
    ...
    try {
      ...
      setError(
        (err as Error).message ? `Failed to delete allocation: ${(err as Error).message}` : "Failed to delete allocation"
      );
      return false;
    } finally {
      setLoading(false);
