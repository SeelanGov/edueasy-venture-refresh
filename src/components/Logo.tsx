...
export const Logo = ({ size = 'medium', layout = 'vertical', className = '' }: LogoProps) => {
      ...
      <div className={`flex items-center gap-3 ${className}`}>
        <div className={`relative ${sizeClasses[size]} flex-shrink-0`}>
          <svg
            ...
            <path d="M120 160C110 150 105 170 115 180C125 190 130 170 120 160Z" fill={designTokens.colors.secondary} />
            <path d="M180 160C190 150 195 170 185 180C175 190 170 170 180 160Z" fill={designTokens.colors.secondary} />
            <path d="M120 170C130 190 140 190 150 170" stroke={designTokens.colors.secondary} strokeWidth="4" />
            <path d="M180 170C170 190 160 190 150 170" stroke={designTokens.colors.secondary} strokeWidth="4" />

            {/* Open book (white) */}
            <path d="M140 195H160C160 195 165 200 150 200C135 200 140 195 140 195Z" fill={designTokens.colors.white} />
            <path d="M140 195V190H160V195" stroke={designTokens.colors.white} strokeWidth="2" />
          </svg>
        </div>
        <div
          className={`font-heading font-bold text-[${designTokens.colors.text.primary}] dark:text-[${designTokens.colors.text.inverse}] ${textSizeClasses[size]}`}
        >
          EduEasy
        </div>
      </div>
  ...
  return (
    <div className={`flex flex-col items-center ${className}`}>
      ...
      {size !== 'small' && (
        <div
          className={`font-heading font-bold text-[${designTokens.colors.text.primary}] dark:text-[${designTokens.colors.text.inverse}] text-center ${textSizeClasses[size]}`}
        >
          EduEasy
        </div>
      )}
