  ...
  return (
    <Card className="p-4 mb-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          {!isLoading && trainingStats && showStats && (
            <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
              <div>
                Training examples: <span className="font-medium">{trainingStats.exampleCount}</span>
              </div>
              <div>
                Confidence threshold:{' '}
                <span className="font-medium">{trainingStats.confidenceThreshold}%</span>
              </div>
              <div>
                Average confidence:{' '}
                <span className="font-medium">{trainingStats.avgConfidence}%</span>
              </div>
              <div>
                Unique intents: <span className="font-medium">{trainingStats.uniqueIntents}</span>
              </div>
            </div>
          )}
        ...
        <Button onClick={handleRetrain} disabled={isRetraining || isLoading} className="relative">
          {isRetraining ? (
            <>
