    ...
    <FormField
      ...
      render={({ field, fieldState }) => (
        <FormItem className="relative">
            ...
            {securityBadgeType && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <span tabIndex={0} aria-label={`${securityBadgeType} badge`} className="outline-none">
                    <SecurityBadge type={securityBadgeType} size="sm" showLabel={false} />
                  </span>
                </TooltipTrigger>
                <TooltipContent>
                  {securityBadgeType === "encryption" && "Encryption: This field is encrypted in storage."}
                  {securityBadgeType === "data-protection" && "Data Protection: This field is protected by data privacy policies."}
                  {securityBadgeType === "privacy" && "Privacy: This field contains private information."}
                  {securityBadgeType === "verification" && "Verification: This field is used for verification purposes."}
                </TooltipContent>
              </Tooltip>
            )}
            {tooltip && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <span tabIndex={0} aria-label={`Info about ${label}`} className="ml-1 cursor-pointer">
          ...
          <FormControl>
            <div className={`relative transition-all duration-200 ${fieldState.invalid ? "border-red-500 bg-red-50 animate-shake" : fieldState.isTouched && !fieldState.invalid ? "border-green-500 bg-green-50" : ""} rounded-md`}> 
              <Input
                {...field}
                type={type}
              ...
              {validationStatus === "valid" && (
                <CheckCircle className="absolute right-2 top-1/2 -translate-y-1/2 text-green-500 h-5 w-5" aria-label="Valid" />
              )}
              {fieldState.invalid && (
                <XCircle className="absolute right-2 top-1/2 -translate-y-1/2 text-red-500 h-5 w-5 animate-shake" aria-label="Invalid" />
              )}
            </div>
          </FormControl>
          ...
          {patternExample && (
            <div className="text-xs text-blue-500 mt-1 animate-fade-in">Example: {patternExample}</div>
          )}
          {maxLength && (
