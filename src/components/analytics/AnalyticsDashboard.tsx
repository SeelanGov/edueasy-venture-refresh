...
import { analyticsService, type ApplicationAnalytics, type RevenueAnalytics, type UserAnalytics } from '@/services/AnalyticsService';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {formatPercentage(applicationAnalytics?.conversion_funnel?.submitted || 0)} success rate
            </p>
          </CardContent>
        </Card>
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary">
                      {userAnalytics?.features_used?.length || 0}
                    </Badge>
                  </div>
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {applicationAnalytics?.conversion_funnel && Object.entries(applicationAnalytics.conversion_funnel).map(([stage, count]) => (
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-2">
                  ...
                  <div className="text-2xl font-bold">{formatNumber(userAnalytics?.total_sessions || 0)}</div>
                  <p className="text-sm text-gray-600">Total sessions</p>
                </div>
                <div className="space-y-2">
                  <h4 className="font-medium">Page Views</h4>
                  <div className="text-2xl font-bold">{formatNumber(userAnalytics?.total_page_views || 0)}</div>
                  <p className="text-sm text-gray-600">Total page views</p>
                </div>
                <div className="space-y-2">
                  <h4 className="font-medium">Conversion Rate</h4>
                  <div className="text-2xl font-bold">{formatPercentage(userAnalytics?.conversion_rate || 0)}</div>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button onClick={() => handleExportReport('application')} variant="outline" size="sm">
                  <Download className="h-4 w-4 mr-2" />
                  Export
                </Button>
              </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-2">
                  ...
                  <div className="text-2xl font-bold">{formatNumber(applicationAnalytics?.total_applications || 0)}</div>
                  <p className="text-sm text-gray-600">All time</p>
                </div>
                <div className="space-y-2">
                  <h4 className="font-medium">Avg Completion Time</h4>
                  <div className="text-2xl font-bold">{Math.round(applicationAnalytics?.average_completion_time || 0)}m</div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-2">
                  ...
                  <div className="text-2xl font-bold">{formatCurrency(revenueAnalytics?.total_revenue || 0)}</div>
                  <p className="text-sm text-gray-600">All time</p>
                </div>
                <div className="space-y-2">
                  <h4 className="font-medium">Avg Order Value</h4>
                  <div className="text-2xl font-bold">{formatCurrency(revenueAnalytics?.average_order_value || 0)}</div>
                  <p className="text-sm text-gray-600">Per transaction</p>
                </div>
                <div className="space-y-2">
                  <h4 className="font-medium">Conversion Rate</h4>
                  <div className="text-2xl font-bold">{formatPercentage(revenueAnalytics?.conversion_rate || 0)}</div>
