          ...
          <tr key={alloc.id}>
            ...
            <td className="px-3 py-2">
              <span className={`px-2 py-1 rounded text-xs ${
                alloc.status === 'active' ? 'bg-green-200 text-green-800' : 'bg-gray-200 text-gray-700'
              }`}>{alloc.status}</span>
            </td>
            <td className="px-3 py-2">{alloc.plan || '-'}</td>
            <td className="px-3 py-2">{alloc.notes || '-'}</td>
            <td className="px-3 py-2 space-x-2">
              <button className="text-blue-600 hover:underline" onClick={() => onEdit(alloc)}>Edit</button>
              <button className="text-red-500 hover:underline" onClick={() => onDelete(alloc.id)}>Delete</button>
            </td>
          </tr>
        ))}
        {allocations.length === 0 && (
          <tr>
            <td colSpan={6} className="text-center text-gray-400 py-8">No allocations found.</td>
          </tr>
        )}
      </tbody>
    </table>
