...
const getStepStatus = (partner: any, key: string) => {
  ...
  switch (key) {
    ...
    case "integration_details":
      return partner.integration_status === "integration_details" || partner.integration_status === "test_api" || partner.integration_status === "completed"
        ? "done" : "pending";
    case "test_api":
      return partner.integration_status === "test_api" || partner.integration_status === "completed"
        ? "done" : "pending";
    case "onboarded":
      return partner.integration_status === "completed"
        ? "done" : "pending";
    default:
      return "pending";
  }
        ...
        {INTEGRATION_STEPS.map(step => {
          ...
          return (
            <li className="flex items-start" key={step.key}>
              {statusToIcon(status)}
              <div>
                <span className={`font-medium ${status === "done" ? "text-green-700" : "text-gray-700"}`}>{step.label}</span>
