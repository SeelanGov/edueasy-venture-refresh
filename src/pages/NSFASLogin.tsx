  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userError || userData?.user_type !== 'nsfas') {
        ...
        throw new Error('Invalid NSFAS credentials');
      }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background-subtle">
      <div className="relative w-full">
        <div className="container mx-auto px-4 mt-8 mb-0 flex items-center">
          <Button
            type="button"
            variant="ghost"
            className="bg-transparent px-3 py-1 rounded-lg flex items-center text-blue-600 hover:bg-blue-600/10"
            onClick={() => navigate('/')}
          >
      ...
      <Card className="bg-card p-8 rounded-xl shadow-lg w-full max-w-md mt-0">
        <h1 className="text-2xl font-bold mb-6 text-center text-blue-600">NSFAS Login</h1>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <Button
            type="submit"
            disabled={loading}
            className="bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg px-5 py-2 mt-2 transition-colors duration-150">
        ...
        <div className="mt-4 text-center text-sm">
          ...
          <Button
            variant="ghost"
            className="text-blue-600 underline p-0 h-auto font-normal"
            onClick={() => navigate('/nsfas/register')}
          >
            Register
          </Button>
        </div>
