  ...
  if (done) {
    return (
      <div className="max-w-md mx-auto mt-16 p-8 bg-white rounded-xl shadow">
        ...
        <button className="text-cap-teal underline" onClick={() => navigate("/")}>Back to Home</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-md mx-auto mt-16 p-8 bg-white rounded-xl shadow">
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        <input required className="input" placeholder="Application ID" value={applicationId} onChange={e=>setApplicationId(e.target.value)} />
        <input required className="input" placeholder="Requested Amount (R)" type="number" value={requestedAmount} onChange={e=>setRequestedAmount(e.target.value)} />
        <input className="input" placeholder="Purpose" value={purpose} onChange={e=>setPurpose(e.target.value)} />
        <button className="w-full bg-cap-teal text-white py-2 rounded" type="submit" disabled={loading}>
